...
| — | [ ] Todo | M07 | Implement one-food feeding and event-driven emotions; newest reaction expires correctly without hidden stats. | M04, M05 | Reaction durations stay configurable/provisional. The emotion half (`emotions.select`) is already implemented in `features/emotions.py` as part of M12 scaffolding; this task now only needs `feeding.decide`. |
