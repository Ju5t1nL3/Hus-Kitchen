...
| — | [ ] Todo | M25 | Replace immediate free Feed with a Feed menu offering Food, Drink and Back; Food/Drink each show a configured yarn cost, atomically spend yarn and feed the pet, and reject insufficient funds without changing state. | M24, M11–M13, user item choices | Prices, names/reactions and whether both use distinct animations remain open. Current direction is immediate purchase-and-feed, not inventory. Coordinate assets/layout with the M16 owner. |
| — | [ ] Blocked | M26 | Define and implement the complete mood catalog, precedence, triggering events and durations; update typed mood/render contracts, rules, fixtures and sprites together. | M25, user mood specification | User will provide every mood, when it occurs and for how long. Preserve no-death/no-care-stat principles. |
| — | [ ] Todo | M27 | Decide and implement base XP and yarn gains from completed focus sessions using a versioned policy and one durable, deduplicated reward breakdown per completion. | M24, M26, user reward choices | Rates, duration scaling, caps and level thresholds remain open. Early endings, pauses and breaks do not qualify unless the user changes that rule. |
| — | [ ] Todo | M28 | Add optional keystroke-count summaries and their additional yarn calculation; count activity only during active focus, never capture keys/text, and verify pause exclusion and opt-out. | M27, user keyboard reward choices | Thresholds/caps remain open. Adapter failure or disabled tracking gives no bonus and never removes base rewards. |
| — | [ ] Todo | M29 | Add optional local camera-attention summaries and their additional yarn calculation; store no frames/identity, distinguish unavailable observations, and verify pause exclusion and opt-out. | M28, user attention reward choices | Attention/coverage thresholds and caps remain open. Camera and keyboard settings stay independent. |
| — | [ ] Todo | M30 | Add yarn bonuses for crossed levels, uninterrupted focus-chain length and the first qualifying completion on a continuing daily streak; persist an explainable deduplicated breakdown and test resets, day boundaries and multi-level gains. | M10, M24, M27–M29, user bonus/streak choices | Exact bonus amounts and focus-chain reset semantics remain open. Daily/weekly facts come from M10; no bonus may be granted twice after restart. |
