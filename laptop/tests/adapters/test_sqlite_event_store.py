...
def every_draft() -> tuple[EventDraft, ...]:
    ...
    return (
        ...
        BreakSessionPaused(
            **metadata("button:c:3"), session_id="break-1", active_ms=10_000
        ),
        FocusSessionResumed(
            **metadata("button:c:4"), session_id="focus-1", active_ms=10_000
        ),
        BreakSessionResumed(
            **metadata("button:c:5"), session_id="break-1", active_ms=10_000
        ),
        FocusSessionCompleted(
...
class SqliteEventStoreTests(unittest.TestCase):
    ...
    def test_every_event_variant_survives_close_and_reopen(self) -> None:
        ...
        self.assertEqual([event.seq for event in restored], list(range(1, 14)))
